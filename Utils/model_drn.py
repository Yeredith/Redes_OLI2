...
#Una capa de upsampling para aumentar la resolución de las imágenes.
class UpSample2D(Layer):
    ...
    def call(self, inputs):
        ...
        return x
#se utiliza para cambiar la media de las imágenes.
class MeanShift(Layer):
    def __init__(self, sign=-1, **kwargs):
        super(MeanShift, self).__init__(**kwargs)
        self.sign = sign
        self.mean_initializer = tf.constant_initializer(value=[[[[0.4488, 0.4371, 0.4040]]]])
        self.mean = self.add_weight(name='mean', shape=(1, 1, 1, 3), initializer=self.mean_initializer, trainable=False)

    def call(self, inputs):
        return tf.math.add(inputs, self.mean * self.sign)
...
#Bloque descendente que reduce la resolución espacial de las características.
def downblock(inputs, nFeat, name):
    ...
    return x
#Bloque ascendente que aumenta la resolución espacial de las características.
def upblock(inputs, nFeat1, nFeat2, scale, name):
    ...
    return x
#Capa que realiza la atención de canal (channel attention).
...
#Bloque residual de atención de canal (channel attention residual block).
def rcab(inputs, nFeat, name):
    ...
    return x
#Bloque que repite rcab para construir una parte de la red DRN.
...
#El modelo dual se utiliza para entrenamiento, generando imágenes de baja resolución (lr_image) 
#y aplicando una red SR a estas imágenes para obtener una versión super-resuelta .
